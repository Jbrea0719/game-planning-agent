...
function MobileChat({ sessionId, nickname, simulateKeyboard }: { sessionId: string; nickname: string; simulateKeyboard?: boolean }) {
  ...
  async function beginInterviewRoom(opts: { label: string; targetSubCategoryId?: string; writingDocId?: string }) {
    ...
    try {
      ...
      // 3. 신규 방으로 전환 (pairs 비움). DB 저장은 전환 후에 해야 중복 안 됨.
      ...
      await fetch("/api/messages", {
        ...
        body: JSON.stringify({
          messages: [
            { session_id: sessionId, pair_id: pairId, role: "user", content: userMsg, conversation_id: convId },
            { session_id: sessionId, pair_id: pairId, role: "assistant", content: question, conversation_id: convId },
          ],
        }),
      }).catch(() => {});
    ...
    try {
      ...
      await fetch("/api/messages", {
        ...
        body: JSON.stringify({
          messages: [
            { session_id: sessionId, pair_id: pairId, role: "user", content: userMsg, conversation_id: convId },
            { session_id: sessionId, pair_id: pairId, role: "assistant", content: intro, conversation_id: convId },
          ],
        }),
      }).catch(() => {});
      ...
      setPairs(prev => [...prev, {
        pair_id: pairId,
        user: { role: "user", content: userMsg },
        assistant: { role: "assistant", content: intro },
      }]);
      // 자동 맥락선 — 이 시점 이후 대화부터 수정 근거로
      setContextAnchor(pairId, new Date().toISOString());
      setTimeout(() => inputRef.current?.focus(), 100);
    } catch (err) {
