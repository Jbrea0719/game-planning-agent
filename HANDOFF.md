...
(참고: 대화 불러오기 100개 제한 버그는 터미널 세션이 `43ed5ea`로 최신 250쌍 로드로 수정·배포 완료. 무한 스크롤(50개+스크롤당 10개, 최대 500쌍) 고도화는 **미완 — 충돌로 중단됨**, 다음에 단독 세션에서 재개 가능.)

---

## 🚧 다음에 할 만한 것 (미정 / 사용자와 상의 필요)

- **대화 무한 스크롤** — 최신 50쌍 먼저 + 위로 스크롤 시 10쌍씩 추가(최대 500쌍), 스크롤 위치 유지. API 1개 + 채팅화면 2개 수정. (충돌로 중단됐던 작업)
