...
## 📅 2026-06-02 할일 (사용자 지정 — 우선)

1. **전체 카테고리 재정비·확정** — 세븐나이츠2 + 세븐나이츠 리버스 개발 문서 토대로 대/중/소 트리 정비·확정. (자료 필요 → 시작 시 사용자에게 요청)
...
- ⚠️ **수동 적용 대기**(Supabase SQL): ① `ALTER TABLE design_docs ADD COLUMN IF NOT EXISTS sort_order INTEGER;`(기획서 드래그 순서 저장) ② '영웅 등급 체계' 소분류 복구
