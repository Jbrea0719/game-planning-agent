...
10. 분량은 내용에 맞춰 — 필요하면 **최대 3만 자까지** 상세하게. 단, 군더더기로 억지로 늘리지 말 것.
11. **"기획 바이블 교차 검증(결과)"·"다음 단계(TODO)" 섹션은 작성하지 않는다.** (원본에 있어도 빼고 반환)
12. 본문 전체에서 긴 줄표(—, em dash)를 쓰지 말고 일반 하이픈(-)을 사용한다.`;
...
export async function POST(request: Request) {
  try {
    ...
    if (apply) {
      const content = stripReviewSections(normalizeDashes((body.content_markdown ?? "").trim()));
    ...
    const revisedMd = stripReviewSections(normalizeDashes(
      res.content
